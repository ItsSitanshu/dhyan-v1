...
>>>>>>> 272124979b9c99ee599241b528c9c3ca1274638f
...
>>>>>>> 272124979b9c99ee599241b528c9c3ca1274638f
  ...
  const addChatToUser = async (userId: string, chatId: string) => {
    ...
    const updatedChats = Array.isArray(userData.chats)
  ...
  const generateTitle = async (chatId: string) => {
    ...
    let conversationContext = responses
      .map((chat) => {
  ...
  const handleResponse = async () => {
    ...
    try {
      let conversationContext = newResponses
        .map((chat) => {
          return `${chat.isUser ? "User" : "Assistant"}: ${chat.message}\n`;
        })
        .join("\n");
      ...
      if (response.code === 200) {
        ...
        const cleanedSpecialAction = response.special_action
          .replace(/json|`|\n/gi, "")
          .trim();
...
=======
  ...
  return !(user && dbUser) ? (
    ...
    dbUser ? (
      <div className="h-screen bg-lprim flex items-center p-4">
        <Sidebar currentPage="chat" />
        <div className="flex bg-opacity-25 w-[82%] h-[94%] rounded-[3rem] bg-bgsec flex-col items-center justify-center relative">
          {showSimulation ? (
            ...
            >
              {responses.map((chat, index) => (
                <div
                  key={index}
                  className={`flex ${chat?.data && "flex-col"} ${
                    chat.isUser ? "justify-end" : "justify-start"
                  }`}
>>>>>>> 272124979b9c99ee599241b528c9c3ca1274638f
                >
                  <div
                    className={`relative px-3 py-3 font-normal rounded-2xl my-2 max-w-[85%] ${
                      !chat.isUser ? "bg-foreground" : ""
                    } bg-opacity-20 z-50`}
                  >
                    {chat.isUser && (
                      <div
                        className="absolute z-[-1] inset-0 rounded-2xl opacity-10"
                        style={{
                          background:
                            "linear-gradient(180deg, var(--prim1) 0%, var(--sec1) 30%, var(--prim2) 100%, var(--sec2) 200%)",
                        }}
                      ></div>
                    )}
                    <ReactMarkdown
                      components={{
                        ...
                        p: ({ node, ...props }) => (
                          <p
                            className="leading-relaxed text-background"
                            {...props}
                          />
                        ),
                        a: ({ node, ...props }) => (
                        ...
                        li: ({ node, ...props }) => (
                          <li className="text-background" {...props} />
                        ),
                        blockquote: ({ node, ...props }) => (
                          <blockquote
                            className="border-l-4 border-gray-500 pl-4 italic text-gray-600"
                            {...props}
                          />
                        ),
                      }}
          ...
          <div className="flex flex-row items-center justify-center space-x-4 rounded-xl w-3/4 h-12  relative">
            <textarea
